...
from src.models import MODEL_TYPES, resize_token_type_embeddings
from src.trainer_mezo import Trainer
...
class DynamicTrainingArguments(TrainingArguments):
    ...
    dpzero: bool = field(
        default=False,
        metadata={
            "help": 'whether to use DPZero in training'}
    )
    dpzero_clip_threshold: float = field(
        default=1.0,
        metadata={"help": "DPZero clip threshold"}
    )
    dp_epsilon: float = field(
        default=6.0,
        metadata={"help": "DP epsilon"}
    )
    dp_delta: float = field(
        default=1e-5,
        metadata={"help": "DP delta"}
    )
    ...
    cone: bool = field(
        default=False,
        metadata={
            "help": 'whether to use CONE in training'}
    )
    cone_theta: float = field(
        default=0.785,
        metadata={"help": "CONE theta value"}
    )
    cone_beta: float = field(
        default=0.5,
        metadata={"help": "CONE beta value"}
    )
...
def main():
    ...
    # Create config
    config_kwargs = {'dpzero': training_args.dpzero,
                     'cone': training_args.cone,
                     'num_k': data_args.num_k,
    ...
    # Training
    if training_args.do_train:
        train_result, _, df, df2, pic_title = trainer.train(model_path=model_args.model_name_or_path if os.path.isdir(model_args.model_name_or_path) else None, test_set = test_dataset)
    ...
    if training_args.do_eval:
        ...
        for eval_dataset in eval_datasets:
            ...
            if eval_dataset.args.task_name == 'mnli':
                pic_title = pic_title + ', acc=' + str(eval_result["eval_mnli/acc"]) + ' seed=' + str(training_args.seed)
            else:
                pic_title = pic_title + ', acc=' + str(eval_result["eval_acc"]) + 'seed=' + str(training_args.seed)
        ...
        for test_dataset in test_datasets:
            ...
            if trainer.is_world_process_zero():
                with open(output_test_file, "w") as writer:
                    ...
                    csv_dir = os.path.join('csvs', test_dataset.args.task_name)
                    os.makedirs(csv_dir, exist_ok=True)
                    if eval_dataset.args.task_name == 'mnli':
                        df.to_csv(os.path.join(csv_dir, f'{pic_title}, tacc={test_result["eval_mnli/acc"]}.csv'), index=False)
                    else:
                        df.to_csv(os.path.join(csv_dir, f'{pic_title}, tacc={test_result["eval_acc"]}.csv'), index=False)
