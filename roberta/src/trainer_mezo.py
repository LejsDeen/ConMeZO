...
class Trainer(LinearHeadTrainer):
    ...
    def get_norm_momentum(self, momentum_vec):
        all_t = torch.cat([t.flatten() for t in momentum_vec])
        return torch.norm(all_t)

    def efficient_perturb_parameters(self, model: nn.Module, random_seed: int, scaling_factor=1):
        torch.manual_seed(random_seed)

        if self.args.cone:
            fac = np.sqrt(self.d) / self.get_norm_momentum(self.momentum)
            for i, (name, param) in enumerate(self.named_parameters_to_optim):
                ...
                z = (np.cos(self.args.cone_theta) * self.momentum[i] * fac + np.sin(self.args.cone_theta) * u)
                param.data = param.data + scaling_factor * z * self.args.zero_order_eps
        else:
            for i, (name, param) in enumerate(self.named_parameters_to_optim):
                u = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
                param.data = param.data + scaling_factor * u * self.args.zero_order_eps
        return model
    ...
    def perturb_parameters(self, model: nn.Module, random_vector=None, scaling_factor=1):
        ...
        for i, (name, param) in enumerate(self.named_parameters_to_optim):
            if name in random_vector:
                u = random_vector[name]
            else:
                u = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
                random_vector[name] = u
            # Cone: Perturb parameters (inefficient)

            if self.args.cone:
                param.data = param.data + scaling_factor * (np.cos(self.args.cone_theta) * self.momentum[i] * fac + np.sin(self.args.cone_theta) * u) * self.args.zero_order_eps
            else:
                param.data = param.data + scaling_factor * u * self.args.zero_order_eps
                ...
                if self.args.zero_order_optim:
                    ...
                    for _ in range(num_zs):
                        ...
                        if self.args.efficient_zero_order:
                            ...
                            if self.args.cone and self.momentum is None:
                        ...
                        if self.args.zero_order_use_trainer_optim:
                            ...
                            for i, (name, param) in enumerate(self.named_parameters_to_optim):
                                ...
                                if param.grad is None:
                                    if self.args.cone:
                                        param.data = projected_grad * (np.cos(self.args.cone_theta) * self.momentum[i] * fac + np.sin(self.args.cone_theta) * u)
                                    else:
                                        param.grad = projected_grad * u
                                else:
                                    if self.args.cone:
                                        param.grad += projected_grad * (np.cos(self.args.cone_theta) * self.momentum[i] * fac + np.sin(self.args.cone_theta) * u)
                                    else:
                                        param.grad += projected_grad * u
                    ...
                    # if not using the trainer, the updates are resampled and directly applied to the parameters
                        ...
                        for i, (name, param) in enumerate(self.named_parameters_to_optim):
                            ...
                            if self.args.cone:
                                param.data = param.data - self.args.learning_rate * projected_grad * (np.cos(self.args.cone_theta) * self.momentum[i] * fac + np.sin(self.args.cone_theta) * u)
                            else:
                                param.data = param.data - self.args.learning_rate * projected_grad * u
                    ...
                    if self.args.cone:
                        ...
                        if self.args.efficient_zero_order:
                            self.update_momentum(random_seed, projected_grad)
                        else:
                            fac = np.sqrt(self.d) / self.get_norm_momentum(self.momentum)
                            
                            for i, (name, param) in enumerate(self.named_parameters_to_optim):
                                z = np.cos(self.args.cone_theta) * self.momentum[i] * fac + np.sin(self.args.cone_theta) * random_vector[name]
                                self.momentum[i] = self.args.cone_beta * self.momentum[i] + (1-self.args.cone_beta) * projected_grad * z
    ...
    def update_momentum(self, random_seed, projected_grad):
        torch.manual_seed(random_seed)
        fac = np.sqrt(self.d) / self.get_norm_momentum(self.momentum)
