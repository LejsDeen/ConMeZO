...
from src.trainer_mezo import OurTrainer
import random
...
class OurArguments(TrainingArguments):
    ...
    # CONE args
    cone: bool = False
    cone_theta: float = 1.3
    cone_beta: float = 0.97

    test_eval_interval: int = 2000
...
class Framework:
    ...
    def train(self, train_samples, eval_samples, test_samples):
        """
        Training function
        """
    ...
    if args.train_set_seed is not None or args.num_train_sets is not None:
        ...
        for train_set_id, train_samples in enumerate(train_sets):
            ...
            if args.trainer != "none":
                ...
                # Training
                framework.train(train_samples, dev_samples if dev_samples is not None else eval_samples, eval_samples) # IMPORTANT
            ...
            if not args.no_eval:
                ...
                if args.task_name == "SST2" or args.task_name == "BoolQ" or args.task_name == "Copa" or args.task_name == "ReCoRD" or args.task_name == "MultiRC" or args.task_name == "WIC" or args.task_name == "RTE":
                    acc = metrics["accuracy"]
                else:
                    acc = metrics["f1"]


                # Add final evaluation result to dataframe
                new_row = pd.DataFrame({
                    'steps': [args.max_steps],
                    'train_loss': [None],
                    'test_acc': [acc]  # New column for test accuracy
                })
                df = pd.concat([df, new_row], ignore_index=True)

                if args.cone:
                    df_name = "Cone_" + str(args.cone_theta) + "_" + str(args.cone_beta) + "_" + str(args.train_set_seed)+ "_" + str(framework.d)
                else:
                    df_name = "MeZO_" + str(args.train_set_seed) + "_" + str(framework.d)
                

                csv_dir = os.path.join("csvs", str(args.task_name))
                os.makedirs(csv_dir, exist_ok=True)
                df.to_csv(os.path.join(csv_dir, f"{df_name}_{args.learning_rate}_{args.max_steps}-run-{acc}.csv"))
