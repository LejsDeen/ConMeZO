...
class OurTrainer(Trainer):
    ...
    def _inner_training_loop(
        ...
        if self.args.dpzero:
            sample_rate = total_train_batch_size / num_examples
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                # DPZero and MeZO added: estimate gradient
                if args.trainer == "zo":
                    if args.cone:
                        tr_loss_step = self.cone_step(model, inputs)
                    elif args.dpzero:
                        tr_loss_step = self.dpzero_step(model, inputs)
                    else:
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    # DPZero and MeZO added: update model with the estimated gradient
                    if args.trainer == "zo":
    ...
    def zo_perturb_parameters(self, random_seed=None, scaling_factor=1):
        ...
        if self.args.cone:
            fac = (np.sqrt(self.d) / self.momentum_norm) * self.cos_theta
        ...
        with torch.no_grad():
            ...
            for i, (name, param) in enumerate(self.named_parameters_to_optim):
                ...
                if self.args.cone:
                    u = torch.normal(mean=0, std=self.sin_theta, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
                    param.data = param.data + scaling_factor * (fac * self.momentum[i] + u) * self.args.zo_eps
                else:
                    u = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
                    param.data = param.data + scaling_factor * u * self.args.zo_eps


    
    def get_norm_momentum(self, momentum_vec):
        with torch.no_grad():
            total = torch.zeros((), dtype=torch.float32, device=momentum_vec[0].device)
            for p in momentum_vec:
                p32 = p.float()
                total.add_(torch.sum(p32 * p32))
        norm = total.sqrt()
        if not torch.isfinite(norm):
            raise ValueError("Non-finite value detected in final momentum norm!")
        return norm
    ...
    def dpzero_step(self, model, inputs):
        """
        Estimate gradient by DPZero with clipping and noise injection. Return the loss from f(theta + z)
        """
        args = self.args
        ...
        self.projected_grad = self.dpzero_clip(loss_diff, self.args.dpzero_clip_threshold).mean()
        self.projected_grad += torch.randn(1).item() * self.dpzero_gaussian_std
    ...
    def cone_step(self, model, inputs):
        ...
        if self.momentum is None:
            self.get_first_momentum(self.zo_random_seed)

        self.momentum_norm = self.get_norm_momentum(self.momentum)
    ...
    def get_first_momentum(self, random_seed: int):
        ...
        for name, param in self.named_parameters_to_optim:
            random_vector.append(torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype))

        self.momentum = random_vector
    ...
    def update_momentum(self):
        ...
        for i, (name, param) in enumerate(self.named_parameters_to_optim):
            u = torch.normal(mean=0, std=self.sin_theta, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
            self.momentum[i] = self.args.cone_beta * self.momentum[i] + (1-self.args.cone_beta) * self.projected_grad * (self.momentum[i] * fac + u)
    ...
    def zo_update(self, model):
        ...
        for i, (name, param) in enumerate(self.named_parameters_to_optim):
            ...
            if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                if self.args.cone:
                    param.data = param.data - self._get_learning_rate() * (self.projected_grad * (np.cos(self.args.cone_theta) * self.momentum[i] * fac + np.sin(self.args.cone_theta) * z) + args.weight_decay * param.data)
                else:
                    param.data = param.data - self._get_learning_rate() * (self.projected_grad * z + args.weight_decay * param.data)
            else:
                if self.args.cone:
                    param.data = param.data - self._get_learning_rate() * self.projected_grad * (np.cos(self.args.cone_theta) * self.momentum[i] * fac + np.sin(self.args.cone_theta) * z)
                else:
